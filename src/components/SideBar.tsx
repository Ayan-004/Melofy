...
function SideBar({collapsed, setCollapsed}: SideBarProps) {
    ...
    return (
        ...
        <div className={`fixed top-0 left-0 z-40 h-full bg-gray-00 border border-gray-300/50 backdrop-blur-xl xl:backdrop-blur-none xl:bg-gray-300 text-black pt-4 shadow-xl w-64 transition-transform duration-300 xl:static xl:translate-x-0  ${collapsed ? '-translate-x-full xl:translate-x-0' : 'translate-x-0'}`}>
            ...
            <div className="font-calsans">
                ...
                <h2 className="sidebar-title text-5xl font-bold">
                   <span className={`hidden sm:inline ${collapsed ? 'hidden' : 'inline'}`}>Melofy</span>
                </h2>
                </div>
            ...
            <ul className="sidebar-menu pt-12 pl-6 md:pl-6 text-2xl font-montserrat-medium text-[#464646]">
                <a href="/" className="menu-item flex items-center hover:bg-[#3d3b3b] backdrop-blur-md gap-2 hover:text-[#cdd084] transition-all hover:rounded-full w-40 cursor-pointer hover:shadow-lg p-3"> 
